<script>
  ...
  document.getElementById("btnEnviar").addEventListener("click", () => {
    const cotizacion = construirCotizacion();
    fetch("https://mail-server-byrb.onrender.com/send-quote", {
      method: "POST",
      headers: {
    ...
    .then(res => res.ok ? alert("📧 Cotización enviada por email") : res.text().then(text => { throw new Error(text) }))
    .catch(err => alert("❌ Error al enviar email: " + err.message));
  });

  // === Aprobar Cotización ===
  document.getElementById("btnAprobar").addEventListener("click", () => {
    alert("✅ Cotización aprobada manualmente.");
  });
  ...
  document.getElementById("btnReiniciar").addEventListener("click", () => {
    ...
    document.getElementById("inputImpuesto").value = "10";
    document.getElementById("resSubtotal").innerText = "0.00";
    document.getElementById("resDescuento").innerText = "0.00";
    document.getElementById("resImpuestos").innerText = "0.00";
